...
import ChevronUp from '~icons/ci/chevron-up'
...
export function RankedDistrictCard({
  ...
  return (
    <div className={cn('w-full bg-white p-4', open ? 'rounded-14' : 'rounded-12', className)}>
      {/* 헤더 (탭 → 펼침 토글) */}
      <button
        type="button"
        onClick={toggle}
      ...
      >
        ...
        {open ? (
          <ChevronUp aria-hidden className="size-6 shrink-0 text-gray-400" />
        ) : (
          <ChevronDown aria-hidden className="size-6 shrink-0 text-gray-400" />
        )}
      </button>
        ...
        <div className="mt-4 flex flex-col gap-4">
          {stats.length > 0 && (
            <div className="flex gap-2.5">
              {stats.map((s) => (
                <div key={s.label} className="flex flex-1 flex-col gap-3 rounded-8 bg-gray-70 p-3">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={toggle}
              className="w-[128px] shrink-0 rounded-8 border border-gray-100 px-4 py-3 text-body-m-semibold text-gray-900"
            >
              접기
            </button>
            <button
              type="button"
              onClick={onViewMap}
              className="flex-1 rounded-8 bg-key px-4 py-3 text-body-m-semibold text-white"
            >
              지도에서 확인하기
            </button>
          </div>
