...
- **배포 모드**: 백엔드 공개배포 전이라 앱은 **MSW 목 데모**로 배포 — Vercel 환경변수 `VITE_ENABLE_MSW=true`. 백엔드 배포되면 `VITE_API_BASE_URL`=실서버 주소로 바꾸고 `VITE_ENABLE_MSW` 제거. (`.env`는 gitignore라 배포 설정은 전부 Vercel 대시보드 환경변수로)
